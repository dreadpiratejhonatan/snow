...
try {
  ...
  if (CONFIG.skins.robertson || CONFIG.enemies.robertson) {
    throw new Error("Robertson deveria ter sido removido do roster");
  }
  player.applySkin("natan");
  if (player.skinId !== "natan") throw new Error("applySkin natan falhou");
  ...
  for (const name of whisperMan) {
    if (!String(name).includes("bebe")) {
      throw new Error(`whisper deveria ser bebe-*: ${name}`);
    }
